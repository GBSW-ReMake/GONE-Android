...
제공된 GONE 가로형 로고를 Android 런처 아이콘과 스플래시 로고로 적용합니다. 아이콘 마스킹으로 로고가 잘리지 않도록 흰 배경과 안전 여백을 포함한 정사각형 아이콘을 각 Android 밀도별 리소스로 생성하고, 스플래시는 시스템 시작 화면과 비슷한 크기의 로고를 유지한 채 페이드아웃 전환을 적용합니다.

## 작업 범위

- 원본 로고의 비율을 유지한 정사각형 런처 아이콘을 생성합니다.
- `mipmap-*`의 기존 `ic_launcher.png`를 Android 표준 해상도로 교체합니다.
- 스플래시 로고의 크기 변화 효과를 제거하고 동일 크기의 페이드 전환을 적용합니다.
...
- [x] 스플래시가 크기 변화 없이 시스템 시작 화면과 자연스럽게 전환됩니다.
