...
class _PointIssueFormPageState extends ConsumerState<PointIssueFormPage> {
  ...
  void initState() {
    super.initState();
    _draft =
        ref.read(pointSystemProvider).drafts[widget.student.id] ??
        const PointIssueDraft();
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: Text(
                  '✓ ${widget.student.name} 학생을 추가했습니다.',
                ...
                child: Row(
                  children: [
                    ...
                    IconButton(
                      onPressed: () {
                        ref
                            .read(pointSystemProvider.notifier)
              ...
              Row(
                children: [
                  Expanded(child: _kind(PointKind.reward)),
                  const SizedBox(width: 12),
                  Expanded(child: _kind(PointKind.penalty)),
                ],
              ),
              ...
              DropdownButtonFormField<int>(
                ...
                items: const [
                  DropdownMenuItem(
                    value: 2,
                    child: Text('[2점] 학교 홍보 활동에 성실히 참여한 학생'),
                  ),
                  DropdownMenuItem(value: 1, child: Text('[1점] 교내 행사에 참여한 학생')),
                  DropdownMenuItem(value: 3, child: Text('[3점] 수업시간 교사지시 불이행')),
                ],
                onChanged: (v) {
                  if (v == null) return;
                  setState(
              ...
              TextField(
                controller: _memo,
                maxLines: 3,
                decoration: _decoration(hint: '선택 사항'),
              ),
              const Spacer(),
              Row(
                children: [
                  OutlinedButton(
                    onPressed: () {
                      ref.read(pointSystemProvider.notifier).clear();
                      Navigator.pop(context);
                    },
                    ...
                    child: FilledButton(
                      onPressed: () {
                        ref
                            ...
                            .saveDraft(
                              widget.student,
                              _draft.copyWith(memo: _memo.text),
                            );
                        Navigator.pop(context);
                      },
  ...
  Widget _kind(PointKind kind) {
    final selected = _draft.kind == kind;
    final color = kind == PointKind.reward
        ? GoneColors.success
        : GoneColors.error;
    return OutlinedButton(
      onPressed: () => setState(() => _draft = _draft.copyWith(kind: kind)),
      style: OutlinedButton.styleFrom(
